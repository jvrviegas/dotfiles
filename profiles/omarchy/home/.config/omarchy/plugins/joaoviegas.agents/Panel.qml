...
Panel {
  ...
  readonly property var sessionSummaries: sessionSummaryRows(providers)
  // The readout is wider than a normal icon slot, so the open-panel marker
  // follows the whole painted control instead of only its center.
  readonly property real openPanelIndicatorWidth: !button.vertical ? button.implicitWidth : 0
  ...
  function currentSessionWindow(p) {
    var windows = limitWindows(p)
    for (var i = 0; i < windows.length; i++)
  ...
  function sessionSummaryRows(providerList) {
    var rows = []
    for (var i = 0; i < providerList.length; i++) {
      var p = providerList[i]
      if (!p || (p.providerId !== "claude" && p.providerId !== "codex")) continue
      var session = currentSessionWindow(p)
      rows.push({
        provider: p,
        remaining: session ? Math.round((1 - clamp(session.percent, 0, 1)) * 100) : -1
      })
    }
    return rows
  }
  ...
  function sessionTooltip() {
    ...
    for (var i = 0; i < sessionSummaries.length; i++) {
      var row = sessionSummaries[i]
      var remaining = row.remaining >= 0 ? row.remaining + "%" : "unknown"
      parts.push(row.provider.providerName + ": " + remaining + " session remaining")
    }
    return parts.join(" · ")
  }
  ...
  WidgetButton {
    ...
    text: "󱚣"
    labelVisible: false
    hasVisualContent: root.sessionSummaries.length > 0
    fixedWidth: vertical ? -1 : usageReadout.implicitWidth + Style.space(12)
    tooltipText: root.sessionTooltip()
    ...
    Row {
      ...
      Repeater {
        ...
        Row {
          id: subscriptionReadout
          required property var modelData
          property var candidates: root.iconCandidatesForProvider(
            modelData.provider, root.bar ? root.bar.background : Color.background)
          ...
          Image {
            ...
            source: subscriptionReadout.candidateIndex < subscriptionReadout.candidates.length
              ? subscriptionReadout.candidates[subscriptionReadout.candidateIndex]
              : ""
            sourceSize.width: Style.font.body * 2
            sourceSize.height: Style.font.body * 2
            fillMode: Image.PreserveAspectFit
            onStatusChanged: if (status === Image.Error
                && subscriptionReadout.candidateIndex < subscriptionReadout.candidates.length)
              Qt.callLater(function() { subscriptionReadout.candidateIndex++ })
          }

          Text {
            anchors.verticalCenter: parent.verticalCenter
            textFormat: Text.PlainText
            text: subscriptionReadout.modelData.remaining >= 0
              ? subscriptionReadout.modelData.remaining + "%"
              : "—"
            color: subscriptionReadout.modelData.remaining >= 0
                && subscriptionReadout.modelData.remaining <= 10
              ? root.urgent
              : root.foreground
