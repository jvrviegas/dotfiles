...
A compact bar readout and one panel for every AI coding subscription on the machine.
The bar shows the Claude and Codex marks beside the percentage remaining in
each provider's current session window. The panel is strictly a display: it watches the usage records that
...
- Bar readout: left = panel, right = launch agent, middle = next subscription.
